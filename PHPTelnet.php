...
class PHPTelnet {
	...
	*/
	...
	function versao($dados){
			...
			$aux = explode('Hardware version', $dados);
			$aux = $aux[1];
			$aux = explode('Software version', $aux);
			$val[1] = $aux[0];
			$val[1] = str_replace(" ", "", $val[1]);
			$val[1] = str_replace(":", "", $val[1]);
			$val[1] = trim($val[1]);
			$aux2 = explode('-', $val[1]);
			$val['hardware_ver'] = $aux2[1];
		
			$aux = $aux[1];
			$aux = explode('Minor version', $aux);
			$val[2] = $aux[0];
			$val[2] = str_replace(" ", "", $val[2]);
			$val[2] = trim($val[2]);
			$val['software_ver'] = str_replace(":", "", $val[2]);
